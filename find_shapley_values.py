...
def conditional_shapley_with_binning(
    ...
    return shapley
#------------------------------
def find_shapley_values_function(event_logs, constraint):
    """
    This function calculates the Shapley values for each constraint based on the event logs.

    Args:
        event_logs (pandas.DataFrame): A DataFrame containing the event logs.
        constraint (dict): A single declarative constraint.

    Returns:
        float: The Shapley value for the given constraint
    """
